...
- [x] 1.3 Add `tests/test_db.py` coverage that a read-only open of a `user_version` 0
...
- [x] 2.1 Add `--dry-run` to the `cli` group callback in `src/ebdx/cli.py`, storing it on
...
- [x] 2.2 Take `@click.pass_context` on `index`, `search`, and `schema` and read the flag;
      verify `discover`, `about`, and `version` are untouched by inspection and that their
      existing tests still pass.

## 3. Dry-run behavior per command

- [x] 3.1 Under `--dry-run`, make `search` and `schema` open read-only and skip
...
- [x] 3.2 Under `--dry-run`, make `index` skip `_ensure_data_dir` and perform no write,
...
- [x] 3.3 Report would-create facts for the data directory, database file, and schema when
...
- [x] 4.1 In `tests/test_cli.py`, assert `ebdx --dry-run index <lib>` against a
...
- [x] 4.2 Assert `ebdx --dry-run index <lib>` against an existing database leaves the
      `books` row count, `PRAGMA user_version`, and the file mtime unchanged while listing
      each file as a would-be insert or update.
- [x] 4.3 Assert `ebdx --dry-run search` and `ebdx --dry-run schema` against a database
...
- [x] 4.4 Assert `--dry-run` changes nothing for `discover`, `about`, and `version`, and
      that dry-run output is still shown under `--quiet`.

## 5. Close out

- [x] 5.1 Run `poe qa` (lint, type, test) and `poe format:check`; verify all pass.
- [x] 5.2 Smoke-test `ebdx --dry-run index /mnt/tank/data/eBooks` against a copy of a real
