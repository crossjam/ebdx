...
- [x] 1.1 Add `src/ebdx/progress.py` owning a single `Console(stderr=True)` and returning it
...
- [x] 1.2 Point the loguru sink in `cli._configure_logging` at that console, decoding each
...
- [x] 2.1 Add a determinate helper that wraps a known sequence of files, labels the bar with
      the current filename, and advances per item; verify it renders a bar on a forced
      terminal console and yields every item.
- [x] 2.2 Add an indeterminate helper for the directory walk that pulses and reports a
      running found-count; verify it yields every path the walk produced.
- [x] 2.3 Make both helpers compute `disable = not enabled or not console.is_terminal` in
...
- [x] 3.4 Carry `quiet` on `ctx.obj` beside `dry_run` and pass `show_progress=not quiet`
      from `index` and `discover`; verify `--quiet` still prints results.

## 4. Acceptance tests

- [x] 4.1 In `tests/test_scanner.py`, assert a forced-terminal console shows a bar for the
...
- [x] 4.2 In `tests/test_cli.py`, assert `index` and `discover` write no progress output to
      stdout under the CliRunner's non-terminal streams, and that `--quiet` suppresses the
      display while results still print.
- [x] 4.3 Assert the summary table survives a run with the display active, and that the
      dry-run label and counts are unchanged.
- [x] 4.4 Assert `discover`'s stdout is byte-identical with and without the display, and
      that `--dry-run discover` stays byte-identical to a plain `discover`.

## 5. Close out

- [x] 5.1 Run `ruff check`, `ruff format --check`, `ty check src/`, and `pytest -q`; verify
      all pass with no test regressed.
- [x] 5.2 Smoke-test `ebdx index` and `ebdx discover` under a pty against a generated
